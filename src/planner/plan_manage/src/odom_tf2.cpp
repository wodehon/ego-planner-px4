...
class MocapLocator : public rclcpp::Node
{
public:
  MocapLocator(std::string name) : Node(name)
  {
    // 创建一个订阅者，订阅"odom"话题的nav_msgs::msg::Odometry类型消息
    odom_subscribe_ = this->create_subscription<nav_msgs::msg::Odometry>(
      "odom", rclcpp::SensorDataQoS(),
      std::bind(&MocapLocator::odom_callback, this, std::placeholders::_1));
...
int main(int argc, char **argv)
{
  ...
  // 创建一个MocapLocator节点
  auto node = std::make_shared<MocapLocator>("fishbot_bringup");
